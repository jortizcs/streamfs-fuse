 ...
 */
int hash_table_add(hash_table_t * table, char * key, size_t key_len, char * value, size_t value_len)
{
 ...
 */
int hash_table_remove(hash_table_t * table, char * key, size_t key_len)
{
 ...
 */
char * hash_table_lookup(hash_table_t * table, char * key, size_t key_len)
{
    size_t hash = HASH(key, key_len);
 ...
 */
int hash_table_has_key(hash_table_t * table, char * key, size_t key_len)
{
    size_t hash = HASH(key, key_len);
    fprintf(stdout,"\t\tSearching for key with hash -> %d\n", (int)hash);
    if (!table->store_house[hash])
    {
        fprintf(stdout,"\t\tKey not found with hash -> %d\n, no entries", (int)hash);
    ...
    while(temp)
    {
        ...
        if(temp)
        {
            if (strcmp(temp->key, key)!=0)
            {
                fprintf(stdout,"\t\tKey Found with hash -> %d\n", (int)hash);
                return 1; // key found
            }
            temp=temp->next;
        }
 ...
 */
size_t hash_table_get_keys(hash_table_t * table, char ** keys)
{
    size_t i = 0;
    size_t count = 0;
    keys = calloc(table->key_count, sizeof(char*));
    for(i=0;i<HASH_LEN;i++)
    {
        if (table->store_house[i])
        {
            keys[count++] = (char*)table->store_house[i];
            hash_table_element_t *temp = table->store_house[i];
            #ifdef DEBUG
            size_t num = 1;
            #endif
