 ...
 * @author Jorge Ortiz -- changed it to char*
 */
#ifndef _HASHTABLE_H
#define _HASHTABLE_H
 ...
 */
uint16_t hash_table_do_hash(char * key, size_t key_len, uint16_t max_key);
 ...
 */
#define HT_ADD(table, key, value) hash_table_add(table, (char *) key, strlen(key), (char *) value, strlen(value))
 ...
 * @returns -1 when no memory
 */
int hash_table_add(hash_table_t *, char *, size_t, char *, size_t);
 ...
 */
#define HT_REMOVE(table, key) hash_table_remove(table, key, strlen(key))
 ...
 * @returns -1 when key is not found
 */
int hash_table_remove(hash_table_t *, char *, size_t);
 ...
 * @returns char* pointer to the value in the table
 */
#define HT_LOOKUP(table, key) hash_table_lookup(table, key, strlen(key))
 ...
 * @returns char* pointer to the value in the table
 */
char * hash_table_lookup(hash_table_t *, char *, size_t);
 ...
 */
#define HT_HAS_KEY(table, key) hash_table_has_key(table, key, strlen(key))
 ...
 * @returns 1 when key is found
 */
int hash_table_has_key(hash_table_t *, char *, size_t);

/**
 * Function to return all the keys in a given hash table
 * @param table hash table from which key are to be reterived
 * @param keys a char** pointer where keys are filled in (memory allocated internally and must be freed)
 * @return total number of keys filled in keys 
 */
size_t hash_table_get_keys(hash_table_t *, char **);
