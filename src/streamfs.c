/*
  ...
  The point of this FUSE filesystem is to provide access to a StreamFS
  instance in order to serve as a familiar abstraction for managing physical
  object and data about the physical world.  The reason it's called StreamFS
  is because file represent streaming data and can be used to pipe data to
  outside processes.
*/
...
int main(int argc, char *argv[])
{
    ...
    sfs_data = calloc(sizeof(struct sfs_state), 1);
    if (sfs_data == NULL) {
	perror("main calloc");
	abort();
    }
    ...
    if ((argc - i) != 2) sfs_usage();
    
    sfs_data->rootdir = realpath(argv[i], NULL);
